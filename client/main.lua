                    ...
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, 74) then
                                TriggerServerEvent('kg-streetraces:JoinRace', k)
                            end
                        end
                    end
                end
            ...
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    ...
                    if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 15.0 then
                        TriggerServerEvent('kg-streetraces:RaceWon', RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('kg-streetraces:StartRace', function(race)
...
RegisterNetEvent('kg-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        KGCore.Functions.Notify('Race Is Over! The Winner Is ' .. winner .. '!')
    end
end)

RegisterNetEvent('kg-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
    SendNUIMessage({
        action = 'HIDE_UI'
...
RegisterNetEvent('kg-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(PlayerPedId(), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - c) > Config.MinimumRaceDistance then
            ...
            TriggerServerEvent('kg-streetraces:NewRace', race)
        else
            KGCore.Functions.Notify('End Position Is Too Close', 'error')
        end
    else
        KGCore.Functions.Notify('You Need To Drop A Marker', 'error')
    end
end)

RegisterNetEvent('kg-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('kg-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
    SendNUIMessage({
        action = 'SHOW_UI'
...
RegisterNetEvent('kg-streetraces:UpdateRaceInfo', function(racers, prizepool)
    SendNUIMessage({
